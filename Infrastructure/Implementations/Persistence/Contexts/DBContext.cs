...
public abstract class DBContext(DbContextOptions options)
        : IdentityDbContext<ApplicationUser, IdentityRole<Guid>, Guid>(options)
{
    ...
    public async Task<TResult?> QueryGetAsync<TResult>(string query, 
    ...
    public async Task<IReadOnlyList<TResult>> QueryListAsync<TResult>(string query,
        CancellationToken cancellationToken)
    {
    ...
    public async Task<TResult?> ExecuteStoredProcedureGetAsync<TResult>(string storedProcedureName, 
        CancellationToken cancellationToken, params SqlParameter[] parameters)
    {
        var result = await ExecuteStoredProcedureListAsync<TResult>(storedProcedureName, 
            cancellationToken, parameters);

        return result.Count > 0 ? result[0] : default;
    }

    public async Task<IReadOnlyList<TResult>> ExecuteStoredProcedureListAsync<TResult>(string storedProcedureName, 
        CancellationToken cancellationToken,
    ...
    public async Task<IReadOnlyList<TResult>> ExecuteRawSqlQueryListAsync<TResult>(string sqlQuery,
        CancellationToken cancellationToken,
        params SqlParameter[] parameters)
    {
        var results = new List<TResult>();
